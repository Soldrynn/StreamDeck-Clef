...
const manifest = JSON.parse(await read("com.davedev.apple-music.sdPlugin", "manifest.json"));
const project = await read("helper", "AppleMusicBridge", "AppleMusicBridge.csproj");
const program = await read("helper", "AppleMusicBridge", "Program.cs");
const pluginReadme = await read("com.davedev.apple-music.sdPlugin", "README.txt");
...
assert.match(pluginReadme, new RegExp(`^Apple Music ${packageJson.version}$`, "m"), "plugin README version");
assert.match(publicReadme, /^# Apple Music for Stream Deck \+$/m, "public README title");
...
for (const name of ["LICENSE", "NOTICE", "THIRD_PARTY_NOTICES.md"]) {
  ...
  const packaged = await read("com.davedev.apple-music.sdPlugin", name);
